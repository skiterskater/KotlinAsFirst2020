...
data class Square(val column: Int, val row: Int) {
     ...
     */
    fun notation(): String = if (!inside()) "" else 'a' - 1 + column + "$row"
}
...
fun square(notation: String): Square =
    if (notation.length == 2 && notation[1] in '1'..'8' && notation[0] in 'a'..'h') Square(
        notation[0] - 'a' + 1,
...
fun rookTrajectory(start: Square, end: Square): List<Square> {
    ...
    if (start.column != end.column && start.row != end.row) result.add(Square(end.column, start.row))
    result.add(end)
    return result
}
...
fun kingMoveNumber(start: Square, end: Square): Int {
    if (!start.inside() || !end.inside()) throw IllegalArgumentException()
    val first = abs(end.row - start.row)
    val second = abs(end.column - start.column)
...
fun kingTrajectory(start: Square, end: Square): List<Square> {
    ...
    var current = start
    while (current != end) {
        val rowMove = if (end.row >= current.row) 1 else -1
        val columnMove = if (end.column >= current.column) 1 else -1
        current = Square(current.column + columnMove, current.row + rowMove)
        result.add(current)
    }
    return result
}
